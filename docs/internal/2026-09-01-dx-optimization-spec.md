...
> `EtherSegmentedControl.SelectionCommand`/`SelectionCommandParameter` are registered in
> `PublicAPI.Unshipped.txt` with a user-initiated-only fixture. Checklist items below are kept
> checked off as a record of what was done; do not re-do this work without a new mandate.
>
...
- [x] **P1.2 — "Binding to a view-model" / MVVM patterns section.** One new section covering: DP TwoWay
...
- [x] **P1.3 — Interactions adapter usage.** A short subsection showing
...
- [x] **P2.1 — Add an `ItemsSource` + TwoWay demo block** to `SegmentedControlPage.xaml`,
...
- [x] **P2.2 — Optional `Command` example on `ButtonPage.xaml`.** One button wired via
...
- [x] **P2.3** Localization: follow the page's existing `x:Uid` convention for any new visible strings
...
- [x] **P3.1 — Add DPs** to `src/Ether.DesignSystem.Controls/Controls/Inputs/EtherSegmentedControl.cs`:
...
- [x] **P3.2 — Fire on user-initiated selection only.** When the selection changes because the **user**
...
- [x] **P3.3 — Register public API:** add the two properties + two DP fields to
  `src/Ether.DesignSystem.Controls/PublicAPI.Unshipped.txt` (the Roslyn analyzer fails the build
  otherwise).
- [x] **P3.4 — Prove it.** Add a fixture (in the SegmentedControl runtime verification, wired into a

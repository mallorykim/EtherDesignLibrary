...
Public API is governed by `PublicAPI.Shipped.txt` and `PublicAPI.Unshipped.txt`. Removing, renaming, or changing the compatible behavior of a shipped public member is breaking and requires a major stable version (or clear preview release notes before stability). Additive public API is normally a minor stable change.

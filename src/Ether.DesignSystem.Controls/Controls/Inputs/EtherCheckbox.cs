...
public sealed class EtherCheckbox : CheckBox
{
    ...
    // This control is intentionally two-state. A null (indeterminate) IsChecked is reachable via
    // a bool? binding regardless of IsThreeState; the default style has no Indeterminate visual,
    // so a null would otherwise strand the control on its previous Unchecked/Checked face. Coerce
    // it back to false so the control always renders a defined state.
    ...
    /// <summary>
    /// DefaultEtherCheckboxStyle's CheckStates group only declares Unchecked and Checked
